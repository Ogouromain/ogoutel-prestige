...
export async function updateSession(request: NextRequest) {
  ...
  const supabaseAnonKey = (process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY || process.env.ANON_KEY || '').trim();
  ...
  try {
    ...
    // ─── Pour les requêtes API avec header Authorization, laisser passer ───
    // L'auth sera vérifiée par verifyApiAuth() dans le route handler.
    // Le middleware Edge de Vercel ne peut pas faire de fetch sortant.
